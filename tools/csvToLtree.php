   ...
   while (($line = stream_get_line($f, 4000, $lf))) {
    ...
    if ($charset != 'utf8') $line=iconv($charset, 'utf8', $line);
		...
		foreach(explode($separator, $line) as $k => $v) {
			...
			if ($data != '') {
				$currentId += 1;
				${'level'.$k.'Id'} = $currentId;
				if ($k == 0) {
					$currentParentId = 'NULL';
				} else {
					$os = $k-1;
					$currentParentId = ((isset(${'level'.$os.'Id'}) ? ${'level'.$os.'Id'} : $currentId));
				}
				$outp .= "($currentId, $currentParentId, '$data'),";
			}
		}
	}
...
$install = <<< INSTALL
CREATE TABLE ${tableName}(${tablePrefix}_id integer PRIMARY KEY, ${tablePrefix}_parentid integer, ${tablePrefix}_name varchar(100), node_path ltree);
...
INSERT INTO ${tableName} (${tablePrefix}_id, ${tablePrefix}_parentid, ${tablePrefix}_name)
	VALUES
	$installContent;
INSTALL;
