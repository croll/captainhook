...
var omniGrid = new Class({
	...
	draw: function(){
		...
		if (this.options.pagination && !this.container.getElement('div.pDiv') )
		{
			...
			pDiv.inject(this.container, this.options.paginationPosition);
			...
			{
				this.options.perPage = isNaN(parseInt(rpObj.options[0].value)) ? 99999999999 : rpObj.options[0].value;
	...
	perPageChange: function(){
		this.options.page = 1;
        var perPage=this.container.getElement('.rp').value;
		this.options.perPage = isNaN(parseInt(perPage)) ? 99999999999 : perPage;
        console.log("perPage: ", this.options.perPage);
		this.refresh();
	},
